...
public class MaxKTrussTSetSubSet extends SparkApp {
    ...
    public Map <Integer, Integer> explore() {
        ...
        Queue<JavaPairRDD <Edge, Integer>> trussQueue = new LinkedList<>();
        ...
        while (true) {
            ...
            if (checkNextSubset) {
                ...
                while (true) {
                    ...
                    minSup = subTSet.filter(kv -> kv._2[0] > 0)
                            .map(kv -> kv._2[0])
                            .reduce(Math::min) + 1;
            ...
            if (trussQueue.size() > 1)
                trussQueue.remove().unpersist();
    ...
    public Tuple2<JavaPairRDD<Edge, Integer>, JavaPairRDD <Edge, int[]>> generate(
        ...
        while (true) {
            ...
            eTruss = eTruss.union(invalids.mapValues(v -> minSup - 1));
