...
//        String inputPath = "input.txt";
        ...
        conf.registerKryoClasses(new Class[]{VertexDegree.class, long[].class});
        conf.set("spark.serializer", "org.apache.spark.serializer.KryoSerializer");
        JavaSparkContext sc = new JavaSparkContext(conf);

        // Load input which is a list of edges separated by a space
        ...
        while (true) {
            ...
            JavaPairRDD<Edge, Integer> triangleEdges = triangles.flatMapToPair(new PairFlatMapFunction<Edge[], Edge, Integer>() {
                @Override
                public Iterable<Tuple2<Edge, Integer>> call(Edge[] t) throws Exception {
                    List<Tuple2<Edge, Integer>> edges = new ArrayList<>(3);
            ...
            JavaPairRDD<Edge, Integer> edgeCount = triangleEdges.reduceByKey((a, b) -> a + b);

            JavaRDD<Edge> newEdges = edgeCount.filter(t -> {
        ...
        System.out.println("Remaining graph edge count: " + currentEdges.count());
        new File(outputPath).delete();
        currentEdges.saveAsTextFile(outputPath);
    }
